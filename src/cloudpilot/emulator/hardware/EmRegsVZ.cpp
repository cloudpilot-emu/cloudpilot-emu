...
void EmRegsVZ::CycleSlowly(Bool sleeping) {
    ...
    if (gSession->HasButtonEvent()) {
        ...
        if (event.GetButton() == ButtonEvent::Button::cradle)
            EmRegsVZ::HotSyncEvent(event.GetType() == ButtonEvent::Type::press);
        else
            EmRegsVZ::DispatchButtonEvent(event);
    }
...
void EmRegsVZ::DispatchButtonEvent(ButtonEvent event) {
...
uint16 EmRegsVZ::ButtonToBits(ButtonEvent::Button button) {
    switch (button) {
...
void EmRegsVZ::UpdateIRQ3(uint8 oldBit) {
    uint8 currentBit = GetPortInputValue('D');
    uint8 pdsel = READ_REGISTER(portDSelect);
    uint16 icr = READ_REGISTER(intControl);
