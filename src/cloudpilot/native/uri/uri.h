   ...
   public:
    ...
    uri(std::map<component, std::string> const &components, scheme_category category,
        ...
        if (category == scheme_category::Hierarchical) {
            ...
            if (components.count(component::Content)) {
                ...
                throw std::invalid_argument(
                    "Content is a required component for a non-hierarchical URI, even an empty "
                    "string.");
            }
        }
    ...
    uri(uri const &other, std::map<component, std::string> const &replacements)
        ...
        if (m_category == scheme_category::Hierarchical) {
            ...
            m_host = (replacements.count(component::Host)) ? replacements.at(component::Host)
                                                           : other.m_host;
            ...
            m_path = (replacements.count(component::Path)) ? replacements.at(component::Path)
                                                           : other.m_path;
        } else {
        ...
        m_query = (replacements.count(component::Query)) ? replacements.at(component::Query)
                                                         : other.m_query;
    ...
    std::string const &get_content() const {
        if (m_category != scheme_category::NonHierarchical) {
            throw std::domain_error(
                "The content component is only valid for non-hierarchical URIs.");
        }
        return m_content;
    };
   ...
   private:
    ...
    std::string::const_iterator parse_content(std::string const &uri_text,
        ...
        if ((m_category == scheme_category::Hierarchical) && (m_content.length() > 0)) {
            // If it's a hierarchical URI, the content should be parsed for the hierarchical
            // components.
