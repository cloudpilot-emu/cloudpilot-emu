...
void EmRegsSZ::CycleSlowly(Bool sleeping) {
    ...
    if (gSession->HasButtonEvent()) {
        ...
        if (event.GetButton() == ButtonEventT::Button::cradle)
            EmRegsSZ::HotSyncEvent(event.GetType() == ButtonEventT::Type::press);
        else
            EmRegsSZ::ButtonEvent(event);
    }
...
void EmRegsSZ::ButtonEvent(ButtonEventT event) {
...
uint16 EmRegsSZ::ButtonToBits(ButtonEventT::Button button) {
    switch (button) {
