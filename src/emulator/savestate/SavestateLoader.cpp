...
bool SavestateLoader::ParseSavestate(void* buffer, size_t size) {
    ...
    if (size < 4) {
        log::printf("buffer is not a valid savestate: too small for header\n");
        return false;
    }
    ...
    if (size < 4 + chunkCount * 8) {
        log::printf("buffer is not a valid savestate: too small for TOC\n");
        return false;
    }
    ...
    for (size_t i = 0; i < chunkCount; i++) {
        ...
        if (size - (nextChunk - static_cast<uint8*>(buffer)) < chunkSize) {
            log::printf("buffer is not a valid savestate: too small for chunk %ul of %ul\n", i,
                        chunkCount);
            return false;
        }
    ...
    if ((nextChunk - static_cast<uint8*>(buffer)) != static_cast<ssize_t>(size)) {
        log::printf("buffer is not a valid savestate: too large\n");
        return false;
    }
...
Chunk* SavestateLoader::GetChunk(ChunkType type) {
    if (chunkMap.find(type) == chunkMap.end()) {
        log::printf("chunk type 0x%04x not in map\n", type);
        return nullptr;
    }
