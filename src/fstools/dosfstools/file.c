...
static void put_char(char **p, unsigned char c, unsigned int out_size) {
    if (dos_char_to_printable(p, c, out_size)) return;
        ...
        for (walk = *current; walk; walk = walk->next)
            if (!here &&
                (!strncmp(name, walk->name, MSDOS_NAME) ||
                 (type == fdt_undelete && !strncmp(name + 1, walk->name + 1, MSDOS_NAME - 1))))
...
FDSC **file_cd(FDSC **curr, char *fixed) {
    ...
    for (walk = curr; *walk; walk = &(*walk)->next)
        if (!strncmp((*walk)->name, fixed, MSDOS_NAME) && (*walk)->first) return &(*walk)->first;
    return NULL;
}
...
static FDSC **file_find(FDSC **dir, char *fixed) {
    ...
    if (*(unsigned char *)fixed == DELETED_FLAG) {
        while (*dir) {
            if (!strncmp((*dir)->name + 1, fixed + 1, MSDOS_NAME - 1) && !(*dir)->first) return dir;
            dir = &(*dir)->next;
        }
        return NULL;
    }
    while (*dir) {
        if (!strncmp((*dir)->name, fixed, MSDOS_NAME) && !(*dir)->first) return dir;
        dir = &(*dir)->next;
    }
    return NULL;
}
...
void file_modify(FDSC **curr, char *fixed) {
    ...
    if (!(this = file_find(curr, fixed))) die("Internal error: file_find failed");
    switch ((*this)->type) {
        case fdt_drop:
...
static void report_unused(FDSC *this) {
    ...
    while (this) {
        ...
        if (this->first)
            ...
            printf("Warning: did not %s file %s\n", this->type == fdt_drop ? "drop" : "undelete",
                   file_name((unsigned char *)this->name));
        free(this);
        this = next;
    }
}

void file_unused(void) { report_unused(fp_root); }
