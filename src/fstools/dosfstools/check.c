...
#define MODIFY(p, i, v)                                                                      \
    do {                                                                                     \
        if (p->offset) {                                                                     \
            p->dir_ent.i = v;                                                                \
            fs_write(p->offset + offsetof(DIR_ENT, i), sizeof(p->dir_ent.i), &p->dir_ent.i); \
        }                                                                                    \
    } while (0)
    ...
    do {                                                                      \
        ...
        if (!p->offset) {                                                     \
            ...
            fs_write(offsetof(struct boot_sector, root_cluster),              \
                     sizeof(((struct boot_sector *)0)->root_cluster), &__v);  \
        } else {                                                              \
            MODIFY(p, start, htole16((__v)&0xffff));                          \
            if (fs->fat_bits == 32) MODIFY(p, starthi, htole16((__v) >> 16)); \
        }                                                                     \
    } while (0)
...
static char *path_name(DOS_FILE *file) {
    ...
    if (!file)
        *path = 0; /* Reached the root directory */
        ...
        if (strlen(path_name(file->parent)) > PATH_NAME_MAX) die("Path name too long.");
...
static char *file_stat(DOS_FILE *file) {
    ...
    secs = (time & 31) * 2;
    if (secs > 59) secs = 59;
    sprintf(temp, "  Size %u bytes, date %02u:%02u:%02u %s %02u %4u", le32toh(file->dir_ent.size),
            hours, minutes, secs, month_str[month - 1], day, year);
    return temp;
}
...
static int bad_name(DOS_FILE *file) {
    ...
    for (i = 0; i < MSDOS_NAME; i++) {
        if ((name[i] < ' ' && !(i == 0 && name[0] == 0x05)) || name[i] == 0x7f) return 1;
    ...
    if (atari_format && suspicious) return 1;
...
static void auto_rename(DOS_FILE *file) {
    ...
    while (1) {
        ...
        for (walk = first; walk; walk = walk->next)
            if (walk != file && !strncmp((const char *)walk->dir_ent.name,
...
static uint32_t scan_free_entry(DOS_FS *fs, DOS_FILE *this) {
    ...
    while (clu_num > 0 && clu_num != -1) {
        ...
        if (IS_FREE(de.name)) return offset;
        i += sizeof(DIR_ENT);
        if (!(i % fs->cluster_size))
            if ((clu_num = next_cluster(fs, clu_num)) == 0 || clu_num == -1) break;
    }
...
static int handle_dot(DOS_FS *fs, DOS_FILE *file, int dotdot) {
    ...
    if (dotdot) {
        ...
        if (!file->parent->parent) {
            ...
            if (start == fs->root_cluster) start = 0;
        }
    } else {
        name = ".";
        ent = MSDOS_DOT;
    ...
    if (!(file->dir_ent.attr & ATTR_DIR) || (FSTART(file, fs) != start) ||
        ...
        if (IS_FREE(file->dir_ent.name)) {
            ...
            switch (get_choice(1, "  Creating.", 2, 1, "Create entry", 2, "Drop parent")) {
                case 1:
                    goto conjure;
                case 2:
...
static int check_file(DOS_FS *fs, DOS_FILE *file) {
    ...
    if (IS_FREE(file->dir_ent.name)) return 0;
    ...
    if (file->dir_ent.attr & ATTR_DIR) {
        ...
        if (file->parent) {
            ...
            if (fs->root_cluster && grandp == fs->root_cluster) grandp = 0;
    ...
    if (FSTART(file, fs) == 1) {
        ...
        if (!file->offset) die("Bad FAT32 root directory! (bad start cluster 1)\n");
        MODIFY_START(file, 0, fs);
    }
    if (FSTART(file, fs) >= fs->data_clusters + 2) {
        printf("%s\n  Start cluster beyond limit (%lu > %lu). Truncating file.\n", path_name(file),
               (unsigned long)FSTART(file, fs), (unsigned long)(fs->data_clusters + 1));
        if (!file->offset)
    ...
    clusters = prev = 0;
    for (curr = FSTART(file, fs) ? FSTART(file, fs) : -1; curr != -1;
        ...
        if (!curEntry.value || bad_cluster(fs, curr)) {
            printf("%s\n  Contains a %s cluster (%lu). Assuming EOF.\n", path_name(file),
                   curEntry.value ? "bad" : "free", (unsigned long)curr);
            if (prev)
                set_fat(fs, prev, -1);
            else if (!file->offset)
        ...
        if (!(file->dir_ent.attr & ATTR_DIR) &&
            ...
            printf(
                ...
                path_name(file), le32toh(file->dir_ent.size),
        ...
        if ((owner = get_owner(fs, curr))) {
            ...
            printf("%s\n  share clusters.\n", path_name(file));
            clusters2 = 0;
            for (walk = FSTART(owner, fs); walk > 0 && walk != -1; walk = next_cluster(fs, walk))
                if (walk == curr)
                    break;
                else {
            ...
            if (!owner->offset) {
                ...
                xasprintf(&trunc_first_string, "Truncate first to %llu bytes (clusters %u)%s",
                          (unsigned long long)clusters2 * fs->cluster_size, clusters2,
                          restart ? " and restart" : "");
                xasprintf(&trunc_second_string, "Truncate second to %llu bytes (clusters %u)",
                          (unsigned long long)clusters * fs->cluster_size, clusters);
                xasprintf(&noninteractive_string,
                          "  Truncating second to %llu bytes (clusters %u).",
                          (unsigned long long)clusters * fs->cluster_size, clusters);

                do_trunc = get_choice(2, noninteractive_string, 2, 1, trunc_first_string, 2,
                                      trunc_second_string);
            ...
            if (do_trunc == 1) {
                prev = 0;
                clusters = 0;
                for (this = FSTART(owner, fs); this > 0 && this != -1;
    ...
    if (!(file->dir_ent.attr & ATTR_DIR) &&
        ...
        printf(
            ...
            "  Truncating file to %u bytes (%u clusters).\n",
            path_name(file), le32toh(file->dir_ent.size),
            (unsigned long long)clusters * fs->cluster_size, new_size, clusters);
...
static int check_dir(DOS_FS *fs, DOS_FILE **root, int dots) {
    ...
    if (*root && parent && good + bad > 4 && bad > good / 2) {
        printf("%s\n  Has a large number of bad entries. (%d/%d)\n", path_name(parent), bad,
               good + bad);
        if (!dots)
            printf("  Not dropping root directory.\n");
        else if (get_choice(2, "  Not dropping it in auto-mode.", 2, 1, "Drop directory", 2,
                            "Keep directory") == 1) {
    ...
    while (*walk) {
        if (!((*walk)->dir_ent.attr & ATTR_VOLUME) && bad_name(*walk)) {
            ...
            switch (get_choice(3, "  Auto-renaming it.", 4, 1, "Drop file", 2, "Rename file", 3,
                               "Auto-rename", 4, "Keep it")) {
                case 1:
                    drop_file(fs, *walk);
                    walk = &(*walk)->next;
                    continue;
            ...
            while (*scan && !skip) {
                if (!((*scan)->dir_ent.attr & ATTR_VOLUME) &&
                    ...
                    printf("%s\n  Duplicate directory entry.\n  First  %s\n", path_name(*walk),
                           file_stat(*walk));
                    printf("  Second %s\n", file_stat(*scan));
                    switch (get_choice(6, "  Auto-renaming second.", 6, 1, "Drop first", 2,
                                       "Drop second", 3, "Rename first", 4, "Rename second", 5,
                                       "Auto-rename first", 6, "Auto-rename second")) {
                        case 1:
                            drop_file(fs, *walk);
                            *walk = (*walk)->next;
                            skip = 1;
...
static void test_file(DOS_FS *fs, DOS_FILE *file, int read_test) {
    ...
    prev = clusters = 0;
    for (walk = FSTART(file, fs); walk > 1 && walk < fs->data_clusters + 2; walk = next_clu) {
        ...
        if ((owner = get_owner(fs, walk))) {
            if (owner == file) {
                printf(
                    ...
                    "cluster%s.\n",
                    path_name(file), (unsigned long)clusters, clusters == 1 ? "" : "s");
                if (prev)
                    set_fat(fs, prev, -1);
                else if (!file->offset)
        ...
        if (read_test) {
            if (fs_test(cluster_start(fs, walk), fs->cluster_size)) {
                ...
                printf("%s\n  Cluster %lu (%lu) is unreadable. Skipping it.\n", path_name(file),
                       (unsigned long)clusters, (unsigned long)walk);
                if (prev)
...
static void undelete(DOS_FS *fs, DOS_FILE *file) {
    ...
    clusters = left = (le32toh(file->dir_ent.size) + fs->cluster_size - 1) / fs->cluster_size;
    prev = 0;
    ...
    if (prev)
        ...
        MODIFY_START(file, 0, fs);
    if (left)
        printf("Warning: Did only undelete %lu of %lu cluster%s.\n", (unsigned long)clusters - left,
               (unsigned long)clusters, clusters == 1 ? "" : "s");
}

static void new_dir(void) { lfn_reset(); }
...
static void add_file(DOS_FS *fs, DOS_FILE ***chain, DOS_FILE *parent, off_t offset, FDSC **cp) {
    ...
    if ((type = file_type(cp, (char *)de.name)) != fdt_none) {
        if (type == fdt_undelete && (de.attr & ATTR_DIR)) die("Can't undelete directories.");
    ...
    *chain = &new->next;
    if (list) {
        ...
        if (new->lfn) printf(" (%s)", file_name(new->dir_ent.name)); /* (8.3) */
        printf("\n");
    }
...
static int scan_dir(DOS_FS *fs, DOS_FILE *this, FDSC **cp) {
    ...
    if (clu_num != 0 && clu_num != -1 && this->offset) {
        ...
        if (handle_dot(fs, &file, 0)) return 1;
        i += sizeof(DIR_ENT);

        file.offset = cluster_start(fs, clu_num) + (i % fs->cluster_size);
        fs_read(file.offset, sizeof(DIR_ENT), &file.dir_ent);
        if (handle_dot(fs, &file, 1)) return 1;
        i += sizeof(DIR_ENT);
    }
    while (clu_num > 0 && clu_num != -1) {
        add_file(fs, &chain, this, cluster_start(fs, clu_num) + (i % fs->cluster_size), cp);
        i += sizeof(DIR_ENT);
        if (!(i % fs->cluster_size))
            if ((clu_num = next_cluster(fs, clu_num)) == 0 || clu_num == -1) break;
    }
...
static char print_fat_dirty_state(void) {
    ...
    return get_choice(1, " Automatically removing dirty bit.", 2, 1, "Remove dirty bit", 2,
                      "No action");
}
...
void check_dirty_bits(DOS_FS *fs) {
    if (fs->fat_bits == 32) {
        ...
        if ((b32.boot_flags & FAT_STATE_DIRTY) ||
            !(fat32_flags.value & FAT32_FLAG_CLEAN_SHUTDOWN)) {
            if (print_fat_dirty_state() == 1) {
                ...
                if (!(fat32_flags.value & FAT32_FLAG_CLEAN_SHUTDOWN)) {
                    ...
                    *new_flags_ptr = htole32(fat32_flags.value | FAT32_FLAG_CLEAN_SHUTDOWN |
                                             (fat32_flags.reserved << 28));
                    fs_write(fs->fat_start + 4, 4, new_flags_ptr);
                    if (fs->nfats > 1) fs_write(fs->fat_start + 4 + fs->fat_size, 4, new_flags_ptr);
                }
            }
        }
    } else {
            ...
            if (print_fat_dirty_state() == 1) {
                ...
                if (fat16_is_dirty) {
                    ...
                    *new_flags_ptr = htole16(fat16_flags.value | FAT16_FLAG_CLEAN_SHUTDOWN);
                    ...
                    if (fs->nfats > 1) fs_write(fs->fat_start + 2 + fs->fat_size, 2, new_flags_ptr);
                }
            }
        }
    }
...
static void get_new_label(char doslabel[12]) {
    ...
    while (1) {
        if (get_line("New label", newlabel, sizeof(newlabel))) {
            ...
            for (i = strlen(doslabel); i < 11; ++i) doslabel[i] = ' ';
            doslabel[11] = 0;
...
static int check_boot_label(DOS_FS *fs) {
    ...
    if (ret & ~0x1) {
        ...
        switch (get_choice(1, "  Auto-removing label from boot sector.", 2, 1,
                           "Remove invalid label from boot sector", 2, "Enter new label")) {
            case 1:
        ...
        printf("Label '%s' stored in boot sector contains lowercase characters.\n",
               pretty_label(fs->label, 0));
        switch (get_choice(1, "  Auto-changing lowercase characters to uppercase", 3, 1,
                           "Change lowercase characters to uppercase", 2, "Remove invalid label", 2,
                           "Set new label")) {
            case 1:
                if (!dos_string_to_wchar_string(wlabel, fs->label, sizeof(wlabel)))
                    die("Cannot change lowercase characters to uppercase.");
                for (i = 0; i < 11; ++i) wlabel[i] = towupper(wlabel[i]);
...
void check_label(DOS_FS *fs) {
    ...
    if (offset == 0 && memcmp(fs->label, "NO NAME    ", 11) != 0) check_boot_label(fs);

    if (offset == 0 && memcmp(fs->label, "NO NAME    ", 11) != 0) {
        printf("Label in boot sector is '%s', but there is no volume label in root directory.\n",
               pretty_label(fs->label, 0));
        switch (get_choice(1, "  Auto-removing label from boot sector.", 2, 1,
                           "Remove label from boot sector", 2,
                           "Copy label from boot sector to root directory")) {
            case 1:
                write_boot_label(fs, "NO NAME    ");
                memcpy(fs->label, "NO NAME    ", 11);
                break;
    ...
    if (offset != 0) {
        ...
        if (doslabel[0] == 0x05) doslabel[0] = 0xe5;
        ret = validate_volume_label(doslabel);
        if (ret & ~0x1) {
            printf("Volume label '%s' stored in root directory is not valid.\n",
                   pretty_label(doslabel, 0));
            switch (get_choice(1, "  Auto-removing label.", 2, 1, "Remove invalid label", 2,
                               "Set new label")) {
                case 1:
                    remove_label(fs);
                    memcpy(fs->label, "NO NAME    ", 11);
            ...
            printf("Volume label '%s' stored in root directory contains lowercase characters.\n",
                   pretty_label(doslabel, 0));
            switch (get_choice(1, "  Auto-changing lowercase characters to uppercase", 3, 1,
                               "Change lowercase characters to uppercase", 2,
                               "Remove invalid label", 2, "Set new label")) {
                case 1:
...
again:
    ...
    if (offset != 0 && memcmp(fs->label, "NO NAME    ", 11) == 0 &&
        ...
        printf(
            "There is no label in boot sector, but there is volume label '%s' stored in root "
            "directory\n",
            pretty_label(doslabel, 0));
        switch (get_choice(1, "  Auto-copying volume label from root directory to boot sector.", 2,
                           1, "Copy volume label from root directory to boot sector", 2,
                           "Remove volume label from root directory")) {
            case 1:
                write_boot_label(fs, doslabel);
                memcpy(fs->label, doslabel, 11);
                break;
    ...
    if (offset != 0 && memcmp(fs->label, "NO NAME    ", 11) != 0 &&
        ...
        printf(
            "Volume label '%s' stored in root directory and label '%s' stored in boot sector and "
            "different.\n",
