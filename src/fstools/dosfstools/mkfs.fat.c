    ...
    switch (size_fat) {
        case 12:
            ...
            if (((cluster * 3) & 0x1) == 0) {
                ...
                fat[3 * cluster / 2] =
                    (unsigned char)((fat[3 * cluster / 2] & 0x000f) | ((value & 0x000f) << 4));
...
static void establish_params(struct device_info *info) {
    ...
    if (info->geom_heads > 0) {
        ...
        if (total_sectors <= 524288) {
            /* For capacity below the 256MB (with 512b sectors) use CHS Recommendation from SD Card
    ...
    if (!bs.secs_track) bs.secs_track = htole16(sec_per_track);
    if (!bs.heads) bs.heads = htole16(heads);
    bs.media = media;
...
static void setup_tables(void) {
    ...
    if (len != (size_t)-1 && len > 11) die("Label can be no longer than 11 characters");

    if (!local_string_to_dos_string(label, volume_name, 12)) die("Error when processing label");

    for (i = strlen(label); i < 11; ++i) label[i] = ' ';
    label[11] = 0;

    if (memcmp(label, "           ", MSDOS_NAME) == 0) memcpy(label, NO_NAME, MSDOS_NAME);
    ...
    if (ret & 0x10) die("Label can't start with a space character");
    ...
    if (!atari_format) {
        ...
        bs.boot_jump[1] =
            ((size_fat == 32 ? (char *)&bs.fat32.boot_code : (char *)&bs.oldfat.boot_code) -
             (char *)&bs) -
            2;
    ...
    if (verbose >= 2) printf("Using %d reserved sectors\n", reserved_sectors);
    ...
    if (!atari_format || size_fat == 32)
        ...
        if (hidden_sectors & ~0xffff) die("#hidden doesn't fit in 16bit field of Atari format\n");
        memcpy(&bs.hidden, &hidden, 2);
    }

    if ((long long)(blocks * BLOCK_SIZE / sector_size) + orphaned_sectors > UINT32_MAX) {
    ...
    if (!atari_format) {
        ...
        do {
            ...
            if (verbose >= 2 && (size_fat == 0 || size_fat == 16))
                printf("Trying FAT16: #clu=%u, fatlen=%u, maxclu=%u, limit=%u/%u\n", clust16,
                       fatlength16, maxclust16, MIN_CLUST_16, MAX_CLUST_16);
            ...
            if (verbose >= 2 && (size_fat == 0 || size_fat == 32))
                printf("Trying FAT32: #clu=%u, fatlen=%u, maxclu=%u, limit=%u/%u\n", clust32,
                       fatlength32, maxclust32, MIN_CLUST_32, MAX_CLUST_32);
        ...
        if (!size_fat) {
            ...
            if (verbose >= 2) printf("Choosing %d bits for FAT\n", size_fat);
        }
        ...
        switch (size_fat) {
            ...
            case 32:
                if (clust32 < MIN_CLUST_32)
                    fprintf(stderr,
                            "WARNING: Number of clusters for 32 bit FAT is less then suggested "
                            "minimum.\n");
         ...
         */
        if (!size_fat)
            size_fat = (num_sectors == 1440 || num_sectors == 2400 || num_sectors == 2880 ||
                        num_sectors == 5760)
                           ? 12
                           : 16;
        if (verbose >= 2) printf("Choosing %d bits for FAT\n", size_fat);
    ...
    if (fill_mbr_partition) {
        ...
        if (verbose) printf("Adding MBR table\n");
        ...
        if (fd >= 0) {
            if (read(fd, buf, sizeof(buf)) == sizeof(buf) && buf[510] == 0x55 && buf[511] == 0xAA)
                disk_sig = (uint32_t)buf[440] | ((uint32_t)buf[441] << 8) |
                           ((uint32_t)buf[442] << 16) | ((uint32_t)buf[443] << 24);
            close(fd);
        }
        ...
        /* Partition type */
        if (le16toh(bs.heads) > 255 ||
            le16toh(bs.secs_track) > 63) { /* CHS values are out of range for MBR, use LBA */
            if (size_fat != 32)
        ...
        if (le16toh(bs.heads) > 255 || le16toh(bs.secs_track) > 63 ||
            ...
            partition[6] =
                ((1 + num_sectors % le16toh(bs.secs_track)) & 63) |
    ...
    if (size_fat == 32) {
        ...
        if (!info_sector) info_sector = 1;
        bs.fat32.info_sector = htole16(info_sector);
        if (!backup_boot_set)
            backup_boot =
                (reserved_sectors >= 7 && info_sector != 6) ? 6
                : (reserved_sectors >= 3 + info_sector && info_sector != reserved_sectors - 2 &&
        ...
        if (verbose >= 2) printf("Using sector %d as backup boot sector (0 = none)\n", backup_boot);
    ...
    if (!cluster_count) {
        if (sectors_per_cluster) /* If yes, die if we'd spec'd sectors per cluster */
            die("Not enough or too many clusters for filesystem - try less or more sectors per "
                "cluster");
        else
    ...
    start_data_sector =
        (reserved_sectors + nr_fats * fat_length + cdiv(root_dir_entries * 32, sector_size)) *
    ...
    if (verbose) {
        ...
        printf("filesystem has %d %d-bit FAT%s and %d sector%s per cluster.\n", (int)(bs.fats),
               size_fat, (bs.fats != 1) ? "s" : "", (int)(bs.cluster_size),
               (bs.cluster_size != 1) ? "s" : "");
        printf("FAT size is %d sector%s, and provides %d cluster%s.\n", fat_length,
               (fat_length != 1) ? "s" : "", cluster_count, (cluster_count != 1) ? "s" : "");
    ...
    if (memcmp(label, NO_NAME, MSDOS_NAME)) {
        ...
        if (de->name[0] == 0xe5) de->name[0] = 0x05;
        de->attr = ATTR_VOLUME;
        ...
        if (ctime && ctime->tm_year >= 80 && ctime->tm_year <= 207) {
            de->time = htole16((unsigned short)((ctime->tm_sec >> 1) + (ctime->tm_min << 5) +
                                                (ctime->tm_hour << 11)));
            de->date = htole16((unsigned short)(ctime->tm_mday + ((ctime->tm_mon + 1) << 5) +
                                                ((ctime->tm_year - 80) << 9)));
        } else {
    ...
    if (size_fat == 32) {
        ...
        if (!(info_sector_buffer = malloc(sector_size))) die("Out of memory");
...
#define seekto(pos, errstr)                       \
    do {                                          \
        off_t __pos = (pos);                      \
        seek = part_sector * sector_size + __pos; \
    } while (0)

#define writebuf(buf, size, errstr)                                                   \
    do {                                                                              \
        int __size = (size);                                                          \
        if (!card_write(seek, __size, buf)) error("could not write to card " errstr); \
        seek += __size;                                                               \
    } while (0)
...
static void write_tables(void) {
    ...
    fat_length = (size_fat == 32) ? le32toh(bs.fat32.fat32_length) : le16toh(bs.fat_length);

    seekto(0, "start of device");
    /* clear all reserved sectors */
    for (x = 0; x < reserved_sectors; ++x) writebuf(blank_sector, sector_size, "reserved sector");
    ...
    if (size_fat == 32) {
        ...
        if (backup_boot != 0) {
            ...
            if (backup_boot + le16toh(bs.fat32.info_sector) != le16toh(bs.fat32.info_sector) &&
                ...
                seekto((backup_boot + le16toh(bs.fat32.info_sector)) * sector_size,
                       "backup info sector");
    ...
    for (x = 1; x <= nr_fats; x++) {
        ...
        for (y = 0; y < blank_fat_length; y++) writebuf(blank_sector, sector_size, "FAT");
    }
...
/* The "main" entry point into the utility - we pick up the options and attempt to process them in
   some sort of sensible way.  In the event that some/all of the options are invalid we need to tell
...
int mkfs_main(int argc, char **argv) {
    ...
    const struct option long_options[] = {{"codepage", required_argument, NULL, OPT_CODEPAGE},
    ...
    if (argc && *argv) { /* What's the program name? */
        char *p;
        program_name = *argv;
        if ((p = strrchr(program_name, '/'))) program_name = p + 1;
    }
    ...
    while ((c = getopt_long(argc, argv, "aAb:cCf:D:F:g:Ii:l:m:M:n:r:R:s:S:h:v", long_options,
                            NULL)) != -1)
        ...
        switch (c) {
            ...
            case 'g': /* g : geometry: heads and sectors per track */
                errno = 0;
                conversion = strtol(optarg, &tmp, 0);
                if (!*optarg || isspace((unsigned char)*optarg) || tmp[0] != '/' || !tmp[1] ||
                    isspace((unsigned char)tmp[1]) || errno || conversion <= 0 ||
            ...
            case OPT_CODEPAGE: /* --codepage : Code page */
                ...
                if (!set_dos_codepage(conversion)) usage(argv[0], 1);
                break;
            ...
            case 's': /* s : Sectors per cluster */
                ...
                conversion = strtol(optarg, &tmp, 0);
                if (!*optarg || isspace((unsigned char)*optarg) || *tmp || errno ||
                    (conversion != 1 && conversion != 2 && conversion != 4 && conversion != 8 &&
                     conversion != 16 && conversion != 32 && conversion != 64 &&
                     conversion != 128)) {
            ...
            case 'S': /* S : Sector size */
                ...
                conversion = strtol(optarg, &tmp, 0);
                if (!*optarg || isspace((unsigned char)*optarg) || *tmp || errno ||
                    (conversion != 512 && conversion != 1024 && conversion != 2048 &&
                     conversion != 4096 && conversion != 8192 && conversion != 16384 &&
                     conversion != 32768)) {
    ...
    if (optind != argc) {
        ...
        if (!*argv[optind] || isspace((unsigned char)*argv[optind]) || *tmp || errno ||
            conversion < 0) {
    ...
    if (create && !blocks_specified) die("Need intended size with -C.");
    ...
    if (!create) {
        ...
        die("create not supported");
    }

    if (get_device_info(&devinfo) < 0) die("error collecting information about %s", device_name);

    if (devinfo.size <= 0) die("unable to discover size of %s", device_name);
    ...
    if (devinfo.sector_size > 0) {
        ...
        if (devinfo.size <= part_sector * sector_size)
            die("The device %s size %llu is less then the offset %llu", device_name, devinfo.size,
    ...
    if (!ignore_safety_checks && devinfo.has_children > 0)
        die("Partitions or virtual mappings on device '%s', not making filesystem (use -I to "
            "override)",
            device_name);
    ...
    /* Let's make sure to sync the block device. Otherwise, if we operate on a loop device and
     * people issue "losetup -d" right after this command finishes our in-flight writes might never
     * hit the disk */
