...
static const char *const cp850_translit_table[128] = {
    "C", "u",  "e",   "a",  "a", "a", "a", "c",  "e",  "e", "e",     "i",   "i", "i",     "A",
    "A", "E",  "ae",  "AE", "o", "o", "o", "u",  "u",  "y", "O",     "U",   "o", "GBP",   "O",
    "x", "f",  "a",   "i",  "o", "u", "n", "N",  "a",  "o", "?",     "(R)", "!", " 1/2 ", " 1/4 ",
    "!", "<<", ">>",  "?",  "?", "?", "|", "+",  "A",  "A", "A",     "(C)", "?", "?",     "?",
    "?", "c",  "JPY", "+",  "+", "+", "+", "+",  "-",  "+", "a",     "A",   "?", "?",     "?",
    "?", "?",  "?",   "?",  "?", "d", "D", "E",  "E",  "E", "i",     "I",   "I", "I",     "+",
    "+", "?",  "?",   "|",  "I", "?", "O", "ss", "O",  "O", "o",     "O",   "u", "th",    "TH",
    "U", "U",  "U",   "y",  "Y", "?", "'", "-",  "+-", "?", " 3/4 ", "?",   "?", "/",     ",",
    "?", "?",  ".",   "1",  "3", "2", "?", " ",
};
...
static int iconv_init_codepage(int codepage, const char *local, iconv_t *to_local,
                               iconv_t *from_local) {
...
static int init_conversion(int codepage) {
    ...
    if (initialized < 0) {
        initialized = 1;
        if (codepage < 0) codepage = DEFAULT_DOS_CODEPAGE;
        ...
        if (initialized) used_codepage = codepage;
    }
    return initialized;
}

int set_dos_codepage(int codepage) { return init_conversion(codepage); }
...
int dos_char_to_printable(char **p, unsigned char c, unsigned int out_size) {
    ...
    if (!init_conversion(-1)) return 0;
    if (!c) return 0;
    if (internal_cp850) return cp850_char_to_printable(p, c, out_size);
...
int local_string_to_dos_string(char *out, char *in, unsigned int out_size) {
    ...
    size_t ret;
    if (!init_conversion(-1)) return 0;
    if (internal_cp850) return local_string_to_cp850_string(out, in, out_size);
    ...
    if (ret == (size_t)-1) {
        if (errno == E2BIG)
            ...
            fprintf(stderr,
                    "Cannot convert input sequence '\\x%.02hhX' from codeset '%s' to 'CP%d': %s\n",
...
int dos_string_to_wchar_string(wchar_t *out, char *in, unsigned int out_size) {
    ...
    size_t ret;
    if (!init_conversion(-1)) return 0;
    if (internal_cp850) return cp850_string_to_wchar_string(out, in, out_size);
...
int wchar_string_to_dos_string(char *out, wchar_t *in, unsigned int out_size) {
    ...
    size_t ret;
    if (!init_conversion(-1)) return 0;
    if (internal_cp850) return wchar_string_to_cp850_string(out, in, out_size);
...
int set_dos_codepage(int codepage) {
    ...
    if (initialized < 0) {
        ...
        if (!initialized)
            fprintf(stderr,
                    "Cannot initialize unsupported codepage %d, only codepage 850 is supported\n",
                    codepage);
    }
    return initialized;
}
