...
int fsck_main(int argc, char **argv) {
    ...
    const struct option long_options[] = {{"variant", required_argument, NULL, OPT_VARIANT},
    ...
    while ((c = getopt_long(argc, argv, "Aac:d:bfF:lnprStu:UvVwy", long_options, NULL)) != -1)
        switch (c) {
            ...
            case 'c':
                ...
                if (!*optarg || isspace((unsigned char)*optarg) || *tmp || errno || codepage < 0 ||
                    codepage > INT_MAX) {
                    fprintf(stderr, "Invalid codepage : %s\n", optarg);
                    usage(argv[0], 2);
                }
                if (!set_dos_codepage(codepage)) usage(argv[0], 2);
                break;
            case 'd':
            ...
            case 'F':
                ...
                if (!*optarg || isspace((unsigned char)*optarg) || *tmp || errno || fat_table < 0 ||
                    fat_table > 255) {
...
exit:
    if (!write_immed && fs_changed()) {
        if (rw) {
            ...
            rw = get_choice(1, "Writing changes.", 2, 1, "Write changes", 2,
    ...
    if (!boot_only)
        printf("%s: %u files, %lu/%lu clusters\n", argv[optind], n_files,
               (unsigned long)fs.data_clusters - free_clusters, (unsigned long)fs.data_clusters);
