...
void get_fat(FAT_ENTRY *entry, void *fat, uint32_t cluster, DOS_FS *fs) {
    ...
    switch (fs->fat_bits) {
        case 12:
            ...
            entry->value =
                0xfff & (cluster & 1 ? (ptr[0] >> 4) | (ptr[1] << 4) : (ptr[0] | ptr[1] << 8));
            break;
        case 16:
...
void release_fat(DOS_FS *fs) {
    if (fs->fat) free(fs->fat);
    if (fs->cluster_owner) free(fs->cluster_owner);
    fs->fat = NULL;
...
static void fix_first_cluster(DOS_FS *fs, void *first_cluster) {
    ...
    if (fs->fat_bits == 12)
        *(uint16_t *)first_cluster =
            htole16((le16toh(*(uint16_t *)first_cluster) & 0xf000) | FAT_EXTD(fs) | b.media);
...
void read_fat(DOS_FS *fs, int mode) {
    ...
    if (fat_table > fs->nfats) die("Requested FAT table %ld does not exist.", fat_table);
    if (fat_table > 2) die("Reading FAT table greather than 2 is not implemented yet.");
    ...
    if (mode != 0 && fat_table == 0) {
        ...
        if (!first_ok && !second)
            die("First FAT appears to be corrupt and second FAT does not exist. Giving up. Run "
                "fsck.fat with -F 1 option.");
    }
    ...
    if (mode != 0 && fat_table == 0 && second && memcmp(first, second, eff_size) != 0) {
        if (mode != 2) die("FATs differ, please run fsck.fat");
    ...
    for (i = 2; i < fs->data_clusters + 2; i++) {
        ...
        if (curEntry.value == 1) {
            if (mode != 2) die("Cluster %ld out of range (1), please run fsck.fat", (long)(i - 2));
            printf("Cluster %ld out of range (1). Setting to EOF.\n", (long)(i - 2));
            set_fat(fs, i, -1);
        }
        if (curEntry.value >= fs->data_clusters + 2 && (curEntry.value < FAT_MIN_BAD(fs))) {
            if (mode != 2)
                die("Cluster %ld out of range (%ld > %ld), please run fsck.fat", (long)(i - 2),
                    (long)curEntry.value, (long)(fs->data_clusters + 2 - 1));
...
void set_fat(DOS_FS *fs, uint32_t cluster, int32_t new) {
    ...
    switch (fs->fat_bits) {
        ...
        case 32: {
            ...
            *(uint32_t *)data = htole32((new & 0xfffffff) | (curEntry.reserved << 28));
            size = 4;
        } break;
        default:
            offs = 0;
...
uint32_t next_cluster(DOS_FS *fs, uint32_t cluster) {
    ...
    if (FAT_IS_BAD(fs, value)) die("Internal error: next_cluster on bad cluster");
...
void fix_bad(DOS_FS *fs) {
    ...
    if (verbose) printf("Checking for bad clusters.\n");
    for (i = 2; i < fs->data_clusters + 2; i++) {
        FAT_ENTRY curEntry;
        get_fat(&curEntry, fs->fat, i, fs);
...
void reclaim_free(DOS_FS *fs) {
    ...
    if (verbose) printf("Checking for unused clusters.\n");
    reclaimed = 0;
    for (i = 2; i < fs->data_clusters + 2; i++) {
        FAT_ENTRY curEntry;
        get_fat(&curEntry, fs->fat, i, fs);

        if (!get_owner(fs, i) && curEntry.value && !FAT_IS_BAD(fs, curEntry.value)) {
            set_fat(fs, i, 0);
            reclaimed++;
        }
    }
...
static void tag_free(DOS_FS *fs, DOS_FILE *owner, uint32_t *num_refs, uint32_t start_cluster) {
    ...
    if (start_cluster == 0) start_cluster = 2;
    ...
    for (i = start_cluster; i < fs->data_clusters + 2; i++) {
        ...
        if (curEntry.value && !FAT_IS_BAD(fs, curEntry.value) && !get_owner(fs, i) &&
            !num_refs[i]) {
            prev = 0;
...
void reclaim_file(DOS_FS *fs) {
    ...
    if (verbose) printf("Reclaiming unconnected clusters.\n");
    ...
    for (i = 2; i < total_num_clusters; i++) {
        ...
        if (!get_owner(fs, i) && next && next < fs->data_clusters + 2) {
             ...
             */
            if (get_owner(fs, next) || !nextEntry.value || FAT_IS_BAD(fs, nextEntry.value))
                set_fat(fs, i, -1);
            else
                num_refs[next]++;
        }
    ...
    do {
        ...
        for (i = 2; i < total_num_clusters; i++) {
            ...
            if (curEntry.value && !FAT_IS_BAD(fs, curEntry.value) && !get_owner(fs, i)) {
                if (!num_refs[curEntry.value]--) die("Internal error: num_refs going below zero");
                set_fat(fs, i, -1);
                 ...
                 */
                if (num_refs[curEntry.value] == 0) break;
            }
        }
    } while (changed);
    ...
    files = reclaimed = 0;
    for (i = 2; i < total_num_clusters; i++) /* If this cluster is the head of an orphan chain... */
        if (get_owner(fs, i) == &orphan && !num_refs[i]) {
            ...
            if (fs->fat_bits == 32) de.starthi = htole16(i >> 16);
            for (walk = i; walk > 0 && walk != -1; walk = next_cluster(fs, walk)) {
    ...
    if (reclaimed)
        printf("Reclaimed %d unused cluster%s (%llu bytes) in %d chain%s.\n", reclaimed,
               reclaimed == 1 ? "" : "s", (unsigned long long)reclaimed * fs->cluster_size, files,
               files == 1 ? "" : "s");
...
uint32_t update_free(DOS_FS *fs) {
    ...
    for (i = 2; i < fs->data_clusters + 2; i++) {
        ...
        if (!get_owner(fs, i) && !FAT_IS_BAD(fs, curEntry.value)) ++free;
    }

    if (!fs->fsinfo_start) return free;

    if (verbose) printf("Checking free cluster summary.\n");
    if (fs->free_clusters != 0xFFFFFFFF) {
        if (free != fs->free_clusters) {
            printf("Free cluster summary wrong (%ld vs. really %ld)\n", (long)fs->free_clusters,
                   (long)free);
            if (get_choice(1, "  Auto-correcting.", 2, 1, "Correct", 2, "Don't correct") == 1)
                do_set = 1;
        }
    } else {
        printf("Free cluster summary uninitialized (should be %ld)\n", (long)free);
        if (rw) {
            if (get_choice(1, "  Auto-setting.", 2, 1, "Set it", 2, "Leave it uninitialized") == 1)
                do_set = 1;
        }
    }
    ...
    if (do_set) {
        ...
        fs_write(fs->fsinfo_start + offsetof(struct info_sector, free_clusters), sizeof(le_free),
                 &le_free);
    }
