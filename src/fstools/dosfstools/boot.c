...
off_t alloc_rootdir_entry(DOS_FS *fs, DIR_ENT *de, const char *pattern, int gen_name) {
    ...
    if (fs->root_cluster) {
        ...
        if (!got) {
             ...
             * after previous one */
            if (!prev) die("Root directory has no cluster allocated!");
            for (clu_num = prev + 1; clu_num != prev; clu_num++) {
                FAT_ENTRY entry;

                if (clu_num >= fs->data_clusters + 2) clu_num = 2;
                get_fat(&entry, fs->fat, clu_num, fs);
                if (!entry.value) break;
            }
            if (clu_num == prev) die("Root directory full and no free cluster");
        ...
        while (next_free < fs->root_entries)
            if (IS_FREE(root[next_free].name) && root[next_free].attr != VFAT_LN_ATTR)
                break;
            else
                next_free++;
        if (next_free == fs->root_entries) die("Root directory is full.");
...
static const char *get_media_descr(unsigned char media) {
    ...
    for (i = 0; i < sizeof(mediabytes) / sizeof(*mediabytes); ++i) {
        if (mediabytes[i].media == media) return (mediabytes[i].descr);
    }
    return ("undefined");
}
...
static void dump_boot(DOS_FS *fs, struct boot_sector *b, unsigned lss) {
    ...
    printf("%10d reserved sector%s\n", le16toh(b->reserved), le16toh(b->reserved) == 1 ? "" : "s");
    ...
    if (!fs->root_cluster) {
        printf("Root directory starts at byte %llu (sector %llu)\n",
               (unsigned long long)fs->root_start, (unsigned long long)fs->root_start / lss);
    ...
    printf("%u sectors/track, %u heads\n", le16toh(b->secs_track), le16toh(b->heads));
    printf("%10u hidden sectors\n",
           atari_format ?
                        /* On Atari, the hidden field is only 16 bit wide and unused */
               (((unsigned char *)&b->hidden)[0] | ((unsigned char *)&b->hidden)[1] << 8)
                        : le32toh(b->hidden));
...
static void check_backup_boot(DOS_FS *fs, struct boot_sector *b, unsigned int lss) {
    ...
    if (!fs->backupboot_start) {
        ...
        if (get_choice(1, "  Auto-creating backup boot block.", 2, 1, "Create one", 2,
            ...
            fs_write(offsetof(struct boot_sector, backup_boot), sizeof(b->backup_boot),
                     &b->backup_boot);
    ...
    if (memcmp(b, &b2, sizeof(b2)) != 0) {
        ...
        pos = 2;
        for (p = (uint8_t *)b, q = (uint8_t *)&b2, i = 0; i < sizeof(b2); ++p, ++q, ++i) {
            if (*p != *q) {
                sprintf(buf, "%s%u:%02x/%02x", first ? "" : ", ", (unsigned)(p - (uint8_t *)b), *p,
                        *q);
        ...
        switch (get_choice(3, "  Not automatically fixing this.", 3, 1, "Copy original to backup",
                           2, "Copy backup to original", 3, "No action")) {
            case 1:
                fs_write(fs->backupboot_start, sizeof(*b), b);
                break;
            case 2:
...
static void read_fsinfo(DOS_FS *fs, struct boot_sector *b, unsigned int lss) {
    ...
    if (!b->info_sector) {
        ...
        if (get_choice(2, "  Not automatically creating it.", 2, 1, "Create one", 2,
                       "Do without FSINFO") == 1) {
            /* search for a free reserved sector (not boot sector and not
             * backup boot sector) */
            uint32_t s;
            for (s = 1; s < le16toh(b->reserved); ++s)
                if (s != le16toh(b->backup_boot)) break;
            if (s > 0 && s < le16toh(b->reserved)) {
                ...
                fs_write(offsetof(struct boot_sector, info_sector), sizeof(b->info_sector),
                         &b->info_sector);
                if (fs->backupboot_start)
                    fs_write(fs->backupboot_start + offsetof(struct boot_sector, info_sector),
    ...
    if (i.magic != htole32(0x41615252) || i.signature != htole32(0x61417272) ||
        i.boot_sign != htole32(0xaa550000)) {
        printf("FSINFO sector has bad magic number(s):\n");
        if (i.magic != htole32(0x41615252))
            printf("  Offset %llu: 0x%08x != expected 0x%08x\n",
                   (unsigned long long)offsetof(struct info_sector, magic), le32toh(i.magic),
                   0x41615252);
        ...
        if (get_choice(1, "  Auto-correcting it.", 2, 1, "Correct", 2,
    ...
    if (fs->fsinfo_start) fs->free_clusters = le32toh(i.free_clusters);
}
...
void read_boot(DOS_FS *fs) {
    ...
    if (position > OFF_MAX) die("Filesystem is too large.");
    ...
    if (!fat_length) die("FAT size is zero.");

    fs->fat_start = (off_t)le16toh(b.reserved) * logical_sector_size;
    position = (le16toh(b.reserved) + b.fats * fat_length) * logical_sector_size;
    if (position > OFF_MAX) die("Filesystem is too large.");
    ...
    if (position > OFF_MAX) die("Filesystem is too large.");
    data_size = position;
    if (data_size < fs->cluster_size) die("Filesystem has no space for any data clusters");
    ...
    if (!b.fat_length && b.fat32_length) {
        ...
        if (fs->data_clusters >= FAT16_THRESHOLD)
            die("Too many clusters (%lu) for FAT16 filesystem.", (unsigned long)fs->data_clusters);
    } else {
        ...
        if (fs->data_clusters + 2 > fat_length * logical_sector_size * 8 / 16 ||
            ...
            (total_sectors == 720 || total_sectors == 1440 || total_sectors == 2880))
            fs->fat_bits = 12;
    }
    ...
    if (position > UINT_MAX) die("FAT has space for too many entries (%lld).", (long long)position);
    total_fat_entries = position;
    if (fs->data_clusters > total_fat_entries - 2)
        die("Filesystem has %u clusters but only space for %u FAT entries.", fs->data_clusters,
            total_fat_entries - 2);
    if (!fs->root_entries && !fs->root_cluster) die("Root directory has zero size.");
...
static void write_boot_label_or_serial(int label_mode, DOS_FS *fs, const char *label,
                                       uint32_t serial) {
    if (fs->fat_bits == 12 || fs->fat_bits == 16) {
        ...
        if (fs->backupboot_start) fs_write(fs->backupboot_start, sizeof(b), &b);
    }
}
...
void write_serial(DOS_FS *fs, uint32_t serial) { write_boot_label_or_serial(0, fs, NULL, serial); }
...
off_t find_volume_de(DOS_FS *fs, DIR_ENT *de) {
    ...
    if (fs->root_cluster) {
        for (cluster = fs->root_cluster; cluster != 0 && cluster != -1;
            ...
            for (i = 0; i * sizeof(DIR_ENT) < fs->cluster_size; i++) {
                ...
                if (!IS_FREE(de->name) && de->attr != VFAT_LN_ATTR && de->attr & ATTR_VOLUME)
                    return offset;
                offset += sizeof(DIR_ENT);
            }
        }
...
void write_volume_label(DOS_FS *fs, char *label) {
    ...
    if (mtime && mtime->tm_year >= 80 && mtime->tm_year <= 207) {
        de.time = htole16(
            (unsigned short)((mtime->tm_sec >> 1) + (mtime->tm_min << 5) + (mtime->tm_hour << 11)));
        de.date = htole16((unsigned short)(mtime->tm_mday + ((mtime->tm_mon + 1) << 5) +
                                           ((mtime->tm_year - 80) << 9)));
    } else {
