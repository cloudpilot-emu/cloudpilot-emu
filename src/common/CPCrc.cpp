...
namespace {
    constexpr uint8_t sdCardCrc7Table[256] = {
    ...
    constexpr uint16_t sdCardCrc16Table[256] = {
...
uint8_t crc::sdCRC7(const uint8_t* data, size_t size) {
    uint8_t crc = 0x00;

    for (size_t offset = 0; offset < size; offset++)
        crc = sdCardCrc7Table[(crc << 1) ^ data[offset]];

    return crc;
}

uint16_t crc::sdCRC16(const uint8_t* data, size_t size) {
    uint16_t crc = 0x0000;

    for (size_t offset = 0; offset < size; offset++)
        crc = sdCardCrc16Table[((crc >> 8) ^ data[offset]) & 0xFF] ^ crc << 8;

    return crc;
}

uint32_t crc::CRC32(const uint8_t* data, size_t size) {
